...
if sum(nu) ~= 0 %Nu is known to the experimenter. 
	...
	if m==0 %Transformation of values to the real line, needed for unconstrained optimisation. 
		zhat = z_trans_m([lambda_start(1:2) lambda_start(3:4) Delta/30 1e-7*FPR],Delta); 
    else 
        zhat = ones(length(fval_exp),3*m+6); 
        fzs = 1:length(fval_exp); 
		for sc=1:length(fval_exp) 
            zhat(sc,:) = z_trans_m([lambda_start(sc,1:end-1) mu_start Delta/30 1e-7*FPR],Delta); %Transformed initialisation to real line. 
    ...
    if m==0 
		zhat = [z_trans_m([lambda_start(1:2) lambda_start(3:4) Delta/30 1e-7*FPR],Delta),z_nu_trans_m(nu_start)]; %Transformation of values to the real line, needed for unconstrained optimisation. 
    else 
        zhat = ones(length(fval_exp),3*m+6); 
        fzs = 1:length(fval_exp); 
		for sc=1:length(fval_exp)
            zhat(sc,:) = [z_trans_m([lambda_start(sc,1:end-1) mu_start Delta/30 1e-7*FPR],Delta),z_nu_trans_m([nu_start 1e-6.*ones(1,m)])]; %Transformed initial

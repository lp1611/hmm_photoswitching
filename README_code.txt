...
COMPILING THE FWDBKWD_NORM_C2.C FILE
- This function is needed to do the forward backward algorithm as is described in the main text 
- As this needs to be compiled, the MATLAB version used (2017a/b) will need to have a gcc compiler installed. 
- To compile: in the main script run the following
mex -O fwdbkwd_norm_c2.c  
This will create an mex executable file.

SIMULATING DATASETS
For the tables (3 and 4 of Appendix B in the main text), we simulated datasets from the model of the 4/5 (m=1,2) state case using the .m file 'SIMX_death_delta_m.m'  
- Each output of this function will be a 1 by lx sample for a SINGLE flurophore (lx is the total number of frames the experiment has run for). 
- Inputs lambda, mu, delta, Delta, lx, nu and false positive rate(see function for more details). 
- Relies on the .m file 'discritiseTransitionTimes.m'
...
NOTE TO ADDRESS USERS WANTING TO REPLICATE THE RESULTS OF THE PAPER 'A hidden Markov model approach to characterizing the
photo-switching behavior of 
fluorophores'
- The matlab file 'CODETOSUBMIT.m' is a sectioned file containing the code needed to obtain the main results of this paper. 
- Please see 'README_data.txt' to ensure that datasets are in the correct folder to run this code without errors.  
